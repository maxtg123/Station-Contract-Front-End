...
import { useDeleteDmLoaiHangMucCshtMutation } from 'src/data/dmLoaiHangMucCsht';
...
type Props = {
  row: IDmLoaiHangMucCsht;
  selected: boolean;
  onEditRow: VoidFunction;
  onSelectRow?: VoidFunction;
  no: number;
};
export default function LoaiHangMucCSHTTableRow({
  no,
  row,
  selected,
  onEditRow,
  ...
  const { mutate: deleteDm, isLoading: deleting } = useDeleteDmLoaiHangMucCshtMutation();
        ...
        { id: row.id.toString() },
        {
          onSuccess: (data) => {
            if (data.status.code === STATUS_CODE_DELETE_ERROR_DANHMUC) {
              enqueueSnackbar(
                `Danh mục Loại Hạng Mục CSHT "${row.ten}" đang được sử dụng nên không thể xóa. Vui lòng kiểm tra dữ liệu`,
                {
                  variant: 'error',
                }
              );
            } else {
              enqueueSnackbar(`Xoá Loại Hạng Mục "${row.ten}" thành công`, {
                variant: 'success',
              });
            }
            handleCloseConfirm();
          ...
          onError: () => {
            enqueueSnackbar(`Có lỗi trong quá trình xoá Loại Hạng Mục`, {
              variant: 'error',
            });
            handleCloseConfirm();
          },
