...
import { useDeleteDmLoaiTramVhktMutation } from 'src/data/dmLoaiTramVhkt';
...
export default function LoaiTramVhktTableRow({ no, row, selected, onEditRow, onSelectRow }: Props) {
  ...
  const { mutate: deleteLoaiTramVhkt, isLoading: deleting } = useDeleteDmLoaiTramVhktMutation();
        ...
        { id: row.id.toString() },
        {
          onSuccess: (data) => {
            if (data.status.code === STATUS_CODE_DELETE_ERROR_DANHMUC) {
              enqueueSnackbar(
                `Danh mục loại trạm vhkt "${row.ten}" đang được sử dụng nên không thể xóa. Vui lòng kiểm tra dữ liệu`,
                {
                  variant: 'error',
                }
              );
            } else {
              enqueueSnackbar(`Xoá Loại trạm VHKT "${row.ten}" thành công`, {
                variant: 'success',
              });
            }
            handleCloseConfirm();
          ...
          onError: () => {
            enqueueSnackbar(`Có lỗi trong quá trình xoá loại trạm vhkt`, {
              variant: 'error',
            });
            handleCloseConfirm();
          },
