...
import HuyenQuanCreate from './HuyenQuanCreate';
import HuyenQuanTableRow from './HuyenQuanTableRow';
import HuyenQuanTableToolbar from './HuyenQuanTableToolbar';
...
const TABLE_HEAD = [
  { id: 'ma', label: 'Mã Quận Huyện', align: 'left' },
  { id: 'ten', label: 'Tên Quận Huyện', align: 'left' },
...
export default function HuyenQuan({ id }: Props) {
  const {
    dense,
    page,
    order,
  ...
  const [tableData, setTableData] = useState<IDmHuyen[]>([]);
  const [dataRow, setDataRow] = useState<IDmHuyen | null>(null);
  const [filterName, setFilterName] = useState('');
  const [openConfirm, setOpenConfirm] = useState(false);
  const [openCreate, setOpenCreate] = useState(false);
  const [filterStatus, setFilterStatus] = useState('');

  const { data, isLoading, isError } = useDmHuyensQuery(id, { refetchOnWindowFocus: false });
  useEffect(() => {
  ...
  return (
    <>
      ...
      {openCreate && (
        <HuyenQuanCreate
          title={dataRow ? 'Cập nhật Huyện / Quận' : 'Thêm mới Huyện / Quận'}
          open={openCreate}
          initData={dataRow}
    ...
    inputData = inputData.filter(
      (user) => {
        ...
        if (!isNil(user?.ghiChu)) {
          ghiChuFilter =
            user.ghiChu.toLowerCase().indexOf(filterNameTrimSpace.toLowerCase()) !== -1;
        }
        return tenFilter || maFilter || ghiChuFilter;
      }

      // || user.ngay_tao.toLowerCase().indexOf(filterName.toLowerCase()) !== -1
    );
  }
  // if (filterStatus !== '') {
