...
import { useDeleteDmLoaiTramMutation } from 'src/data/dmLoaiTram';
...
export default function LoaiTramTableRow({ no, row, selected, onEditRow, onSelectRow }: Props) {
  ...
  const { mutate: deleteLoaiTram, isLoading: deleting } = useDeleteDmLoaiTramMutation();
        ...
        { id: row.id.toString() },
        {
          onSuccess: (data) => {
            if (data.status.code === STATUS_CODE_DELETE_ERROR_DANHMUC) {
              enqueueSnackbar(
                `Danh mục loại trạm "${row.ten}" đang được sử dụng nên không thể xóa. Vui lòng kiểm tra dữ liệu`,
                {
                  variant: 'error',
                }
              );
            } else {
              enqueueSnackbar(`Xoá Loại Trạm "${row.ten}" thành công`, {
                variant: 'success',
              });
            }
            handleCloseConfirm();
          ...
          onError: () => {
            enqueueSnackbar(`Có lỗi trong quá trình xoá loại trạm`, {
              variant: 'error',
            });
            handleCloseConfirm();
          },
