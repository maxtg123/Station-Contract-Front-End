...
import { useDeleteDmTramKhuVucMutation } from 'src/data/dmTramKhuVuc';
...
export default function KhuVucTramTableRow({ no, row, selected, onEditRow, onSelectRow }: Props) {
  ...
  const { mutate: deleteKhuVucTram, isLoading: deleting } = useDeleteDmTramKhuVucMutation();
        ...
        { id: row.id.toString() },
        {
          onSuccess: (data) => {
            if (data.status.code === STATUS_CODE_DELETE_ERROR_DANHMUC) {
              enqueueSnackbar(
                `Danh mục khu vực trạm "${row.ten}" đang được sử dụng nên không thể xóa. Vui lòng kiểm tra dữ liệu`,
                {
                  variant: 'error',
                }
              );
            } else {
              enqueueSnackbar(`Xoá khu vực trạm "${row.ten}" thành công`, {
                variant: 'success',
              });
            }
            handleCloseConfirm();
          ...
          onError: () => {
            enqueueSnackbar(`Có lỗi trong quá trình xoá khu vực trạm`, {
              variant: 'error',
            });
            handleCloseConfirm();
          },
