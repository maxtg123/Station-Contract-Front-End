...
import { useDeleteDmLoaiCotAntenMutation } from 'src/data/dmLoaiCotAnten';
...
export default function LoaiCotAntenTableRow({ no, row, selected, onEditRow, onSelectRow }: Props) {
  ...
  const { mutate: deleteLoaiCotAnten, isLoading: deleting } = useDeleteDmLoaiCotAntenMutation();
        ...
        { id: row.id.toString() },
        {
          onSuccess: (data) => {
            if (data.status.code === STATUS_CODE_DELETE_ERROR_DANHMUC) {
              enqueueSnackbar(
                `Danh mục loại cột Anten "${row.ten}" đang được sử dụng nên không thể xóa. Vui lòng kiểm tra dữ liệu`,
                {
                  variant: 'error',
                }
              );
            } else {
              enqueueSnackbar(`Xoá Loại cột Anten "${row.ten}" thành công`, {
                variant: 'success',
              });
            }
            handleCloseConfirm();
          ...
          onError: () => {
            enqueueSnackbar(`Có lỗi trong quá trình xoá loại cột Anten`, {
              variant: 'error',
            });
            handleCloseConfirm();
          },
