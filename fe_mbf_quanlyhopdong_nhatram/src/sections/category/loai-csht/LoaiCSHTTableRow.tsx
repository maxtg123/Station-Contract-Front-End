...
import { useDeleteDmLoaiCshtMutation } from 'src/data/dmLoaiCsht';
...
export default function LoaiCSHTTableRow({ no, row, selected, onEditRow, onSelectRow }: Props) {
  ...
  const { mutate: deleteLoaiCsht, isLoading: deleting } = useDeleteDmLoaiCshtMutation();
        ...
        { id: row.id.toString() },
        {
          onSuccess: (data) => {
            if (data.status.code === STATUS_CODE_DELETE_ERROR_DANHMUC) {
              enqueueSnackbar(
                `Danh mục loại CSHT "${row.ten}" đang được sử dụng nên không thể xóa. Vui lòng kiểm tra dữ liệu`,
                {
                  variant: 'error',
                }
              );
            } else {
              enqueueSnackbar(`Xoá Loại CSHT "${row.ten}" thành công`, {
                variant: 'success',
              });
            }
            handleCloseConfirm();
          ...
          onError: () => {
            enqueueSnackbar(`Có lỗi trong quá trình xoá loại CSHT`, {
              variant: 'error',
            });
            handleCloseConfirm();
          },
