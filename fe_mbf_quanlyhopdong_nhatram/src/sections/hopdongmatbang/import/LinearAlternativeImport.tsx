...
import { useImportHopDongMutation } from 'src/data/hopDongMatBang';
...
import { transformDataReadFileExcel } from 'src/utils/hopDongUtils';
...
export default function LinearAlternativeImport({ onClose, onStepFinal }: Props) {
  ...
  const { mutate: importHopDong } = useImportHopDongMutation();
  ...
  useMemo(async () => {
    if (file !== null) {
      ...
      const transformedData = transformDataReadFileExcel(result);
        ...
        onSuccess(res) {
          ...
          if (resIdImport)
            addBgProcess({
              ...
              messageSuccess: tienTrinhMessage.HOP_DONG.IMPORT.MAT_BANG.SUCCESS,
              messageError: tienTrinhMessage.HOP_DONG.IMPORT.MAT_BANG.ERROR,
              messageLoading: tienTrinhMessage.HOP_DONG.IMPORT.MAT_BANG.LOADING,
            });
        },
      ...
      <ConfirmDialog
        ...
        content={
          <>
            ...
            <span style={{ color: 'blue', fontWeight: 'bold' }}>mặt bằng</span> không. Đây là import
            hợp đồng <span style={{ color: 'blue', fontWeight: 'bold' }}>mặt bằng</span>. Bạn hãy
            kiểm tra kỹ lại một lần nữa?
          </>
        }
        action={
          <LoadingButton
