...
const SPACING = 2.5;
const DRAG_TRAM_COL = 'dragColumnHopDong';
...
export default function DragColumnTableHopDong({
  open,
  onClose,
  title,
  data,
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    localStorage.setItem(DRAG_TRAM_COL, JSON.stringify(reorderedItems));
  ...
  const handleToggleChecked = (itemId: string) => {
    ...
    localStorage.setItem(DRAG_TRAM_COL, JSON.stringify(updatedItems));
    setItems(updatedItems);
    onDragColumn(updatedItems);
  };
  useEffect(() => {
    const localStorageValue = localStorage.getItem(DRAG_TRAM_COL);
