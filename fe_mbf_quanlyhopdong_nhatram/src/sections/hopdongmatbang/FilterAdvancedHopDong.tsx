...
const TINH_TRANG_HD_OPTS: OptionTypeTram[] = [
  { id: 'KY_MOI', ten: 'Ký mới' },
  { id: 'TAI_KY', ten: 'Tái ký' },
  { id: 'DI_DOI', ten: 'Di dời' },
  { id: 'THANH_LY', ten: 'Thanh lý' },
];

export default function FilterAdvancedHopDong({ open, onClose, title }: Props) {
    ...
    >
      ...
      <Scrollbar sx={{ p: SPACING, pb: 0 }}>
        ...
        <Stack direction="column" mb={3}>
          <Autocomplete
            fullWidth
            options={TINH_TRANG_HD_OPTS}
            value={filterFormFields.tinhTrangHopDong}
            getOptionLabel={(option: OptionTypeTram | string) => (option as OptionTypeTram).ten}
            isOptionEqualToValue={(option, value) => option.id === value.id}
            onChange={(e, newValue) => {
              dispatch({
                type: 'set-form-filter',
                payload: { ...filterFormFields, tinhTrangHopDong: newValue },
              });
            }}
            renderInput={(params) => <TextField {...params} label="Tình trạng hợp đồng" />}
            renderOption={(props, option) => (
              <li {...props} key={option.id}>
                {option.ten}
              </li>
        ...
        <Stack direction="column" mb={3} spacing={3}>
          <DatePicker
            ...
            label="Ngày ký hợp đồng từ ngày"
            onChange={(newValue) => {
              dispatch({
                type: 'set-form-filter',
          ...
          <DatePicker
            ...
            label="Ngày ký hợp đồng đến ngày"
            onChange={(newValue) => {
              dispatch({
                type: 'set-form-filter',
        ...
        <Stack direction="column" mb={3} spacing={3}>
          <DatePicker
            ...
            label="Ngày kết thúc hợp đồng từ ngày"
            onChange={(newValue) => {
              dispatch({
                type: 'set-form-filter',
          ...
          <DatePicker
            ...
            label="Ngày kết thúc hợp đồng đến ngày"
            onChange={(newValue) => {
              dispatch({
                type: 'set-form-filter',
        ...
        <Stack direction="row" mb={2}>
          <Autocomplete
            fullWidth
            options={pdOptions || []}
            value={filterFormFields.phongDaiId}
            getOptionLabel={(option: OptionTypeTram | string) => (option as OptionTypeTram).ten}
            isOptionEqualToValue={(option, value) => option.id === value.id}
            onChange={(e, newValue) => {
              dispatch({
                type: 'set-form-filter',
                payload: { ...filterFormFields, phongDaiId: newValue, to: null },
              });
            }}
