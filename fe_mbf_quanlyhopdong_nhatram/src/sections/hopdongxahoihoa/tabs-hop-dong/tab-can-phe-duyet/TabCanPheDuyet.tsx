...
import { HEAD_HOP_DONG_XA_HOI_HOA_DA_GUI_PHE_DUYET } from 'src/constants/hopdongxahoihoa.constant';
import { useHopDongXaHoiHoaQuery } from 'src/data/hopDongXaHoiHoa';
import Row from './Row';
import Toolbar from './Toolbar';

const TabCanPheDuyet = () => {
  const [tableData, setTableData] = useState<IHopDongXaHoiHoa[]>([]);
  ...
  const { data, isLoading, isError } = useHopDongXaHoiHoaQuery(
    {
      page,
      size: rowsPerPage,
        ...
        <TableContainer sx={{ position: 'relative', overflow: 'unset' }}>
          <Scrollbar>
            <Table size={dense ? 'small' : 'medium'} sx={{ minWidth: 800 }}>
              <TableHeadCustom
                headLabel={HEAD_HOP_DONG_XA_HOI_HOA_DA_GUI_PHE_DUYET}
