...
import {
  TABLE_HEADER_IMPORT_HOP_DONG_XA_HOI_HOA_GROUP_COLUMN,
  TABLE_HEADER_IMPORT_HOP_DONG_XA_HOI_HOA_GROUP_COLUMN_DATA_VALID,
  TABLE_HEAD_IMPORT_HOP_DONG_XA_HOI_HOA,
...
export default function TableCustomImport({ data, onDataValid }: Props) {
  ...
  const { data: listDataHopDong, refetch: refetchHopDong } = useHopDongXaHoiHoaQuery(
      ...
      if (data) {
        ...
        if (tempData.length) {
          ...
          const transformDataImport = filterDataValid.map((item) => {
            const result = getTransFormDataImportHopDong(item, dataDanhMuc, 'XA_HOI_HOA');
            return result;
          });
        ...
        <TableContainer sx={{ maxHeight: 640 }}>
          {/* <TableSelectedAction
            ...
            onSelectAllRows={(checked) =>
              onSelectAllRows(
                checked,
                dataFiltered.map((row) => row.dongSo.toString())
                )
              }
          ...
          <Table size={dense ? 'small' : 'medium'} stickyHeader sx={{ minWidth: 800 }}>
            <TableHeadCustomColumnGroup
              ...
              headLabel={
                ...
                transFormData.filter((item) => item.errors.message.length).length
                  ? TABLE_HEAD_IMPORT_HOP_DONG_XA_HOI_HOA
                  : TABLE_HEAD_IMPORT_HOP_DONG_XA_HOI_HOA.filter((item) => item.id !== 'chiTietLoi')
              }
              isNumIndex
              isStickyActionHead
              rowCount={dataFiltered.length}
                ...
                .map((row, index) => (
                  <CheckImportHopDongTableRow
                    headLabel={
                      ...
                      transFormData.filter((item) => item.errors.message.length).length
                        ? TABLE_HEAD_IMPORT_HOP_DONG_XA_HOI_HOA
                        : TABLE_HEAD_IMPORT_HOP_DONG_XA_HOI_HOA.filter(
                            (item) => item.id !== 'chiTietLoi'
                          )
                    }
                    no={index}
