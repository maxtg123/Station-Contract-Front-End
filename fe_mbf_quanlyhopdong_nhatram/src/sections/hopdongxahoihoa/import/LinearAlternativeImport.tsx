...
import { useImportHopDongXaHoiHoaMutation } from 'src/data/hopDongXaHoiHoa';
...
import { transformDataReadFileExcelHopDongXaHoiHoa } from 'src/utils/hopDongUtils';
...
export default function LinearAlternativeImport({ onClose, onStepFinal }: Props) {
  ...
  const { mutate: importHopDong } = useImportHopDongXaHoiHoaMutation();
  ...
  useMemo(async () => {
    if (file !== null) {
      ...
      const transformedData = transformDataReadFileExcelHopDongXaHoiHoa(result);
        ...
        onSuccess(res) {
          ...
          if (resIdImport)
            addBgProcess({
              ...
              messageSuccess: tienTrinhMessage.HOP_DONG.IMPORT.XA_HOI_HOA.SUCCESS,
              messageError: tienTrinhMessage.HOP_DONG.IMPORT.XA_HOI_HOA.ERROR,
              messageLoading: tienTrinhMessage.HOP_DONG.IMPORT.XA_HOI_HOA.LOADING,
            });
        },
      ...
      <ConfirmDialog
        ...
        content={
          <>
            ...
            <span style={{ color: 'blue', fontWeight: 'bold' }}>xã hội hóa</span> không. Đây là
            import hợp đồng <span style={{ color: 'blue', fontWeight: 'bold' }}>xã hội hóa</span>.
            Bạn hãy kiểm tra kỹ lại một lần nữa?
          </>
        }
        action={
          <LoadingButton
