...
import { PATH_DASHBOARD, PATH_HOP_DONG_IBC } from 'src/routes/paths';
...
const HopDongIbcImport = dynamic(() => import('./HopDongIbcImport'));
...
export default function HopDongIbcContainer() {
  ...
  const { data: listHopDongHienHuu } = useHopDongIbcQuery(
    {
      page: 0,
      size: 1,
  ...
  const { data: listHopDongNhap } = useHopDongIbcQuery(
    {
      page: 0,
      size: 1,
  ...
  const { data: listHopDongDaGuiPheDuyet } = useHopDongIbcQuery(
    {
      page: 0,
      size: 1,
  ...
  const handleChangeTab = (val: ITabValue) => {
    if (activeTab !== val) {
      ...
      router.push(`/hop-dong-ibc/danh-sach/?tab=${val}`);
    }
  };
  return (
    <>
      ...
      <Container maxWidth={themeStretch ? false : 'xl'}>
        <CustomBreadcrumbs
          heading="Danh sách hợp đồng IBC"
          links={[
            { name: 'Quản lý hợp đồng nhà trạm', href: PATH_DASHBOARD.root },
            { name: 'Hợp đồng IBC', href: PATH_HOP_DONG_IBC.root },
            { name: 'Danh sách' },
          ]}
          action={
      ...
      {openImport && (
        <HopDongIbcImport
          title="Import hợp đồng IBC từ file Excel"
          open={openImport}
