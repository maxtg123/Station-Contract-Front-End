...
import { IHopDongIbc, IHopDongIbcForm } from 'src/@types/hopdongibc';
...
import { DRAG_HOP_DONG_IBC_COL, TABLE_HEAD_HOP_DONG_IBC } from 'src/constants/hopdongibc.constant';
import {
  tienTrinhAction,
  tienTrinhMessage,
  tienTrinhModule,
} from 'src/constants/tientrinh.constant';
import { useHopDongIbcFilterContext } from 'src/context/hop-dong-ibc/hopDongIbcFilterContext';
import { useListHopDongIbcContext } from 'src/context/hop-dong-ibc/listHopDongIbcContext';
import { useHopDongIbcQuery } from 'src/data/hopDongIbc';
...
import HopDongIbcTableRow from '../../HopDongIbcTableRow';
import HopDongIbcTableToolbar from '../../HopDongIbcTableToolbar';

const FilterAdvancedHopDongIbc = dynamic(() => import('../../FilterAdvancedHopDongIbc'));
const DragColumnTableHopDongIbc = dynamic(() => import('../../DragColumnTableHopDongIbc'));
const GET_ALL = 10000;
...
const TabActiveOrDraft = ({ type, module }: Props) => {
  ...
  const {
    ...
    dispatch,
  } = useHopDongIbcFilterContext();
  const {
    dispatch: dispatchTab,
    state: { activeTab },
  } = useListHopDongIbcContext();
  const storedData = localStorage.getItem(DRAG_HOP_DONG_IBC_COL);
  const initialHeaderTable = storedData ? JSON.parse(storedData) : TABLE_HEAD_HOP_DONG_IBC;
  const [activeTableData, setActiveTableData] = useState<IHopDongIbc[]>([]);
  const [draftTableData, setDraftTableData] = useState<IHopDongIbc[]>([]);
  ...
  const [dataRow, setDataRow] = useState<IHopDongIbc | null>(null);
  const [updateData, setUpdateData] = useState<IHopDongIbcForm | null>(null);
  ...
  const { data, isLoading, isError, isFetching } = useHopDongIbcQuery(
    {
  ...
  const isFilteredActiveHopDong = filterName !== '' || filteHopDongIbcContext;

  const isFilteredDraftHopDong = filterNameDraftHopDong !== '' || filteHopDongIbcContext;
  const isNotFound =
    type === 'active'
  ...
  const handleEditRow = async (row: IHopDongIbc) => {
    // setDataRow(row);
  ...
  const handleGiaoViecDialog = (row: IHopDongIbc) => {
                ...
                <TableNoData
                  title={
                    !isFilteredActiveHopDong
                      ? 'Dữ liệu hợp đồng IBC đang trống'
