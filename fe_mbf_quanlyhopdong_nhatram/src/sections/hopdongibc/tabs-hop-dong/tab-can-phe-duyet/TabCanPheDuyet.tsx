...
import { HEAD_HOP_DONG_IBC_DA_GUI_PHE_DUYET } from 'src/constants/hopdongibc.constant';
import { useHopDongIbcQuery } from 'src/data/hopDongIbc';
import Row from './Row';
import Toolbar from './Toolbar';

const TabCanPheDuyet = () => {
  const [tableData, setTableData] = useState<IHopDongIbc[]>([]);
  ...
  const { data, isLoading, isError } = useHopDongIbcQuery(
    {
      page,
      size: rowsPerPage,
        ...
        <TableContainer sx={{ position: 'relative', overflow: 'unset' }}>
          <Scrollbar>
            <Table size={dense ? 'small' : 'medium'} sx={{ minWidth: 800 }}>
              <TableHeadCustom
                headLabel={HEAD_HOP_DONG_IBC_DA_GUI_PHE_DUYET}
              ...
              <TableBody>
                ...
                <TableNoData
                  title={
                    !isFiltered
                      ? 'Dữ liệu cần phê duyệt hợp đồng mặt bằng đang trống'
...
export default memo(TabCanPheDuyet);
