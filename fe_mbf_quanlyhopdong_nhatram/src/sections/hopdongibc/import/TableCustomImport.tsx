...
import {
  TABLE_HEADER_IMPORT_HOP_DONG_IBC_GROUP_COLUMN,
  TABLE_HEADER_IMPORT_HOP_DONG_IBC_GROUP_COLUMN_DATA_VALID,
  TABLE_HEAD_IMPORT_HOP_DONG_IBC,
} from 'src/constants/hopdongibc.constant';
...
export default function TableCustomImport({ data, onDataValid }: Props) {
  ...
  const { data: listDataHopDong, refetch: refetchHopDong } = useHopDongIbcQuery(
      ...
      if (data) {
        ...
        if (tempData.length) {
          ...
          const transformDataImport = filterDataValid.map((item) => {
            const result = getTransFormDataImportHopDong(item, dataDanhMuc, 'IBC');
            return result;
          });
          ...
          <Table size={dense ? 'small' : 'medium'} stickyHeader sx={{ minWidth: 800 }}>
            <TableHeadCustomColumnGroup
              ...
              headLabel={
                ...
                transFormData.filter((item) => item.errors.message.length).length
                  ? TABLE_HEAD_IMPORT_HOP_DONG_IBC
                  : TABLE_HEAD_IMPORT_HOP_DONG_IBC.filter((item) => item.id !== 'chiTietLoi')
              }
              isNumIndex
              isStickyActionHead
