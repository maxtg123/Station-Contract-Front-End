...
import KhuVucTramCreate from 'src/sections/category/khu-vuc-tram/KhuVucTramCreate';
import KhuVucTramTableRow from 'src/sections/category/khu-vuc-tram/KhuVucTramTableRow';
import KhuVucTramTableToolbar from 'src/sections/category/khu-vuc-tram/KhuVucTramTableToolbar';
...
export default function LoaiTramPage() {
  ...
  const { data, isLoading, isError, isFetching } = useDmTramKhuVucsQuery({
  ...
  return (
    <>
      <Head>
        <title>Danh mục | Khu vực trạm</title>
      </Head>
      <Container maxWidth={themeStretch ? false : 'xl'}>
        <CustomBreadcrumbs
          heading="Khu vực trạm"
          links={[{ name: 'Danh mục', href: PATH_CATEGORY.root }, { name: 'Khu vực trạm' }]}
          action={<AddNewButtonDanhMuc onOpenPopover={handleOpenPopover} />}
        />
        <Card>
          <KhuVucTramTableToolbar
                ...
                <Table size={dense ? 'small' : 'medium'} sx={{ minWidth: 800 }}>
                  ...
                  <TableBody>
                    ...
                    <TableNoData
                      title={
                        !isFiltered
                          ? 'Dữ liệu danh mục khu vực trạm đang trống'
      ...
      {openCreate && (
        <KhuVucTramCreate
          title={dataRow ? 'Cập nhật Khu vực trạm' : 'Thêm mới Khu vực trạm'}
          open={openCreate}
          initData={dataRow}
