...
import LoaiHangMucCSHTCreate from 'src/sections/category/loai-hang-muc-csht/LoaiHangMucCSHTCreate';
import LoaiHangMucCSHTTableRow from 'src/sections/category/loai-hang-muc-csht/LoaiHangMucCSHTTableRow';
import LoaiHangMucCSHTTableToolbar from 'src/sections/category/loai-hang-muc-csht/LoaiHangMucCSHTTableToolbar';
...
export default function LoaiCSHTPage() {
  ...
  const { data, isLoading, isError, isFetching } = useDmLoaiHangMucCshtsQuery({
    refetchOnWindowFocus: false,
  });

  const { themeStretch } = useSettingsContext();
  const [openPopover, setOpenPopover] = useState<HTMLElement | null>(null);
  const [tableData, setTableData] = useState<IDmLoaiHangMucCsht[]>([]);
  const [dataRow, setDataRow] = useState<IDmLoaiHangMucCsht | null>(null);
  ...
  const handleEditRow = (row: IDmLoaiHangMucCsht) => {
    setDataRow(row);
    setOpenCreate(true);
  };
  return (
    <>
      <Head>
        <title>Danh mục | Loại Hạng Mục CSHT</title>
      </Head>
      <Container maxWidth={themeStretch ? false : 'xl'}>
        <CustomBreadcrumbs
          heading="Loại Hạng Mục CSHT"
          links={[{ name: 'Danh mục', href: PATH_CATEGORY.root }, { name: 'Loại Hạng Mục CSHT' }]}
          action={<AddNewButtonDanhMuc onOpenPopover={handleOpenPopover} />}
        />
        <Card>
          <LoaiHangMucCSHTTableToolbar
                ...
                <Table size={dense ? 'small' : 'medium'} sx={{ minWidth: 800 }}>
                  ...
                  <TableBody>
                    ...
                    <TableNoData
                      title={
                        !isFiltered
                          ? 'Dữ liệu loại hạng mục cơ sở hạ tầng đang trống'
                      ...
                      handleOpenImport={handleClickOpenImport}
                    />
                  </TableBody>
                </Table>
              </Scrollbar>
      ...
      {openCreate && (
        <LoaiHangMucCSHTCreate
          title={dataRow ? 'Cập nhật Loại Hạng Mục CSHT' : 'Thêm mới Loại Hạng Mục CSHT'}
          open={openCreate}
          initData={dataRow}
