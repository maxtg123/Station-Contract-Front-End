...
import HinhThucDauTuCreate from 'src/sections/category/hinh-thuc-dau-tu/HinhThucDauTuCreate';
import HinhThucDauTuTableRow from 'src/sections/category/hinh-thuc-dau-tu/HinhThucDauTuTableRow';
import HinhThucDauTuTableToolbar from 'src/sections/category/hinh-thuc-dau-tu/HinhThucDauTuTableToolbar';
...
// ----------------------------------------------------------------------
HinhThucDTPage.getLayout = (page: React.ReactElement) => <DashboardLayout>{page}</DashboardLayout>;
// ----------------------------------------------------------------------

export default function HinhThucDTPage() {
  ...
  const [tableData, setTableData] = useState<IDmHinhThucDauTu[]>([]);
  const [dataRow, setDataRow] = useState<IDmHinhThucDauTu | null>(null);
  const [filterName, setFilterName] = useState('');
  const [openConfirm, setOpenConfirm] = useState(false);
  const [openCreate, setOpenCreate] = useState(false);
  const [filterStatus, setFilterStatus] = useState('');
  const { data, isLoading, isError, isFetching } = useDmHinhThucDauTusQuery({
  ...
  return (
    <>
      <Head>
        <title>Danh mục | Hình thức đầu tư</title>
      </Head>
      <Container maxWidth={themeStretch ? false : 'xl'}>
        <CustomBreadcrumbs
          heading="Hình thức đầu tư"
          links={[{ name: 'Danh mục', href: PATH_CATEGORY.root }, { name: 'Hình thức đầu tư' }]}
          action={<AddNewButtonDanhMuc onOpenPopover={handleOpenPopover} />}
        />
        <Card>
          <HinhThucDauTuTableToolbar
                ...
                <Table size={dense ? 'small' : 'medium'} sx={{ minWidth: 800 }}>
                  ...
                  <TableBody>
                    ...
                    <TableNoData
                      title={
                        !isFiltered
                          ? 'Dữ liệu hình thức đầu tư đang trống'
      ...
      {openCreate && (
        <HinhThucDauTuCreate
          title={dataRow ? 'Cập nhật Hình thức đầu tư' : 'Thêm mới Hình thức đầu tư'}
          open={openCreate}
          initData={dataRow}
