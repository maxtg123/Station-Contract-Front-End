...
import { IDmLoaiTram, IDmLoaiTramVhkt } from 'src/@types/category';
...
import LoaiTramVhktCreate from 'src/sections/category/loai-tram-vhkt/LoaiTramVhktCreate';
import LoaiTramVhktTableRow from 'src/sections/category/loai-tram-vhkt/LoaiTramVhktTableRow';
import LoaiTramVhktTableToolbar from 'src/sections/category/loai-tram-vhkt/LoaiTramVhktTableToolbar';
...
LoaiTramVhktPage.getLayout = (page: React.ReactElement) => (
  <DashboardLayout>{page}</DashboardLayout>
);

// ----------------------------------------------------------------------

export default function LoaiTramVhktPage() {
  const {
    dense,
    page,
    order,
  ...
  const { data, isLoading, isError, isFetching } = useDmLoaiTramVhktsQuery({
    refetchOnWindowFocus: false,
  });

  const { themeStretch } = useSettingsContext();
  const [openPopover, setOpenPopover] = useState<HTMLElement | null>(null);
  const [tableData, setTableData] = useState<IDmLoaiTramVhkt[]>([]);
  const [dataRow, setDataRow] = useState<IDmLoaiTramVhkt | null>(null);
  ...
  return (
    <>
      <Head>
        <title>Danh mục | Loại trạm VHKT</title>
      </Head>
      <Container maxWidth={themeStretch ? false : 'xl'}>
        <CustomBreadcrumbs
          heading="Loại trạm VHKT"
          links={[{ name: 'Danh mục', href: PATH_CATEGORY.root }, { name: 'Loại trạm VHKT' }]}
          action={<AddNewButtonDanhMuc onOpenPopover={handleOpenPopover} />}
        />
        <Card>
          <LoaiTramVhktTableToolbar
                ...
                <Table size={dense ? 'small' : 'medium'} sx={{ minWidth: 800 }}>
                  ...
                  <TableBody>
                    ...
                    <TableNoData
                      title={
                        !isFiltered
                          ? 'Dữ liệu loại trạm vhkt đang trống'
      ...
      {openCreate && (
        <LoaiTramVhktCreate
          title={dataRow ? 'Cập nhật Loại trạm vhkt' : 'Thêm mới Loại trạm vhkt'}
          open={openCreate}
          initData={dataRow}
