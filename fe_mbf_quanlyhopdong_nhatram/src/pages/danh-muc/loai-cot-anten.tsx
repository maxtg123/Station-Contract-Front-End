...
import LoaiCotAntenCreate from 'src/sections/category/loai-cot-anten/LoaiCotAntenCreate';
import LoaiCotAntenTableRow from 'src/sections/category/loai-cot-anten/LoaiCotAntenTableRow';
import LoaiCotAntenTableToolbar from 'src/sections/category/loai-cot-anten/LoaiCotAntenTableToolbar';
...
// ----------------------------------------------------------------------
LoaiCotAntenPage.getLayout = (page: React.ReactElement) => (
  <DashboardLayout>{page}</DashboardLayout>
);
// ----------------------------------------------------------------------

export default function LoaiCotAntenPage() {
  ...
  const [tableData, setTableData] = useState<IDmLoaiCotAnten[]>([]);
  const [dataRow, setDataRow] = useState<IDmLoaiCotAnten | null>(null);
  const [filterName, setFilterName] = useState('');
  const [openConfirm, setOpenConfirm] = useState(false);
  const [openCreate, setOpenCreate] = useState(false);
  const [filterStatus, setFilterStatus] = useState('');

  const { data, isLoading, isError, isFetching } = useDmLoaiCotAntensQuery({
  ...
  return (
    <>
      <Head>
        <title>Danh mục | Loại cột Anten</title>
      </Head>

      <Container maxWidth={themeStretch ? false : 'xl'}>
        <CustomBreadcrumbs
          heading="Loại cột Anten"
          links={[{ name: 'Danh mục', href: PATH_CATEGORY.root }, { name: 'Loại cột Anten' }]}
          action={<AddNewButtonDanhMuc onOpenPopover={handleOpenPopover} />}
        />
        <Card>
          <LoaiCotAntenTableToolbar
                ...
                <Table size={dense ? 'small' : 'medium'} sx={{ minWidth: 800 }}>
                  ...
                  <TableBody>
                    ...
                    <TableNoData
                      title={
                        !isFiltered
                          ? 'Dữ liệu loại cột anten đang trống'
      ...
      {openCreate && (
        <LoaiCotAntenCreate
          title={dataRow ? 'Cập nhật Loại cột Anten' : 'Thêm mới Loại cột Anten'}
          open={openCreate}
          initData={dataRow}
