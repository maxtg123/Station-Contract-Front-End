...
import { IDmKhoanMuc, IDmLoaiCsht } from 'src/@types/category';
...
import KhoanMucCreate from 'src/sections/category/khoan-muc/KhoanMucCreate';
import KhoanMucTableRow from 'src/sections/category/khoan-muc/KhoanMucTableRow';
import KhoanMucTableToolbar from 'src/sections/category/khoan-muc/KhoanMucTableToolbar';
...
KhoanMucPage.getLayout = (page: React.ReactElement) => <DashboardLayout>{page}</DashboardLayout>;

// ----------------------------------------------------------------------

export default function KhoanMucPage() {
  const {
    dense,
    page,
    order,
  ...
  const { data, isLoading, isError, isFetching } = useDmKhoanMucQuery({
    refetchOnWindowFocus: false,
  });

  const { themeStretch } = useSettingsContext();
  const [openPopover, setOpenPopover] = useState<HTMLElement | null>(null);
  const [tableData, setTableData] = useState<IDmKhoanMuc[]>([]);
  const [dataRow, setDataRow] = useState<IDmKhoanMuc | null>(null);
  ...
  return (
    <>
      <Head>
        <title>Danh mục | Khoản mục</title>
      </Head>
      <Container maxWidth={themeStretch ? false : 'xl'}>
        <CustomBreadcrumbs
          heading="Khoản mục"
          links={[{ name: 'Danh mục', href: PATH_CATEGORY.root }, { name: 'Khoản mục' }]}
          action={<AddNewButtonDanhMuc onOpenPopover={handleOpenPopover} />}
        />
        <Card>
          <KhoanMucTableToolbar
      ...
      {openCreate && (
        <KhoanMucCreate
          title={dataRow ? 'Cập nhật Khoản mục' : 'Thêm mới Khoản mục'}
          open={openCreate}
          initData={dataRow}
