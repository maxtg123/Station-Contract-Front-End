...
import { IDmLoaiCsht, IDmLoaiThietBiRan } from 'src/@types/category';
...
import LoaiThietBiRanCreate from 'src/sections/category/loai-thiet-bi-ran/LoaiThietBiRanCreate';
import LoaiThietBiRanTableRow from 'src/sections/category/loai-thiet-bi-ran/LoaiThietBiRanTableRow';
import LoaiThietBiRanTableToolbar from 'src/sections/category/loai-thiet-bi-ran/LoaiThietBiRanTableToolbar';
...
LoaiThietBiRanPage.getLayout = (page: React.ReactElement) => (
  <DashboardLayout>{page}</DashboardLayout>
);

// ----------------------------------------------------------------------

export default function LoaiThietBiRanPage() {
  const {
    dense,
    page,
    order,
  ...
  const { data, isLoading, isError, isFetching } = useDmLoaiThietBiRansQuery({
    refetchOnWindowFocus: false,
  });

  const { themeStretch } = useSettingsContext();
  const [openPopover, setOpenPopover] = useState<HTMLElement | null>(null);
  const [tableData, setTableData] = useState<IDmLoaiThietBiRan[]>([]);
  const [dataRow, setDataRow] = useState<IDmLoaiThietBiRan | null>(null);
  ...
  return (
    <>
      <Head>
        <title>Danh mục | Loại thiết bị ran</title>
      </Head>
      <Container maxWidth={themeStretch ? false : 'xl'}>
        <CustomBreadcrumbs
          heading="Loại thiết bị RAN"
          links={[{ name: 'Danh mục', href: PATH_CATEGORY.root }, { name: 'Loại thiết bị RAN' }]}
          action={<AddNewButtonDanhMuc onOpenPopover={handleOpenPopover} />}
        />
        <Card>
          <LoaiThietBiRanTableToolbar
                ...
                <Table size={dense ? 'small' : 'medium'} sx={{ minWidth: 800 }}>
                  ...
                  <TableBody>
                    ...
                    <TableNoData
                      title={
                        !isFiltered
                          ? 'Dữ liệu loại thiết bị ran đang trống'
      ...
      {openCreate && (
        <LoaiThietBiRanCreate
          title={dataRow ? 'Cập nhật Loại thiết bị ran' : 'Thêm mới Loại thiết bị ran'}
          open={openCreate}
          initData={dataRow}
