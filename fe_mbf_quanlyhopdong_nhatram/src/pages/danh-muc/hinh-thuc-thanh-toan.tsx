...
import { IDmHinhThucThanhToan, IDmLoaiCsht } from 'src/@types/category';
...
import HinhThucThanhToanCreate from 'src/sections/category/hinh-thuc-thanh-toan/HinhThucThanhToanCreate';
import HinhThucThanhToanTableRow from 'src/sections/category/hinh-thuc-thanh-toan/HinhThucThanhToanTableRow';
import HinhThucThanhToanTableToolbar from 'src/sections/category/hinh-thuc-thanh-toan/HinhThucThanhToanTableToolbar';
...
HinhThucThanhToanPage.getLayout = (page: React.ReactElement) => (
...
export default function HinhThucThanhToanPage() {
  ...
  const { data, isLoading, isError, isFetching } = useDmHinhThucThanhToansQuery({
    refetchOnWindowFocus: false,
  });

  const { themeStretch } = useSettingsContext();
  const [openPopover, setOpenPopover] = useState<HTMLElement | null>(null);
  const [tableData, setTableData] = useState<IDmHinhThucThanhToan[]>([]);
  const [dataRow, setDataRow] = useState<IDmHinhThucThanhToan | null>(null);
    ...
    <>
      ...
      <Container maxWidth={themeStretch ? false : 'xl'}>
        <CustomBreadcrumbs
          ...
          links={[{ name: 'Danh mục', href: PATH_CATEGORY.root }, { name: 'Hình thức thanh toán' }]}
          action={<AddNewButtonDanhMuc onOpenPopover={handleOpenPopover} />}
        />
        <Card>
          <HinhThucThanhToanTableToolbar
                ...
                <Table size={dense ? 'small' : 'medium'} sx={{ minWidth: 800 }}>
                  ...
                  <TableBody>
                    ...
                    <TableNoData
                      title={
                        !isFiltered
                          ? 'Dữ liệu hình thức thanh toán đang trống'
      ...
      {openCreate && (
        <HinhThucThanhToanCreate
          title={dataRow ? 'Cập nhật Hình thức thanh toán' : 'Thêm mới Hình thức thanh toán'}
          open={openCreate}
          initData={dataRow}
...
function applyFilter({
  ...
  inputData: IDmHinhThucThanhToan[];
