...
import ToCreate from 'src/sections/category/to/ToCreate';
import ToTableRow from 'src/sections/category/to/ToTableRow';
import ToTableToolbar from 'src/sections/category/to/ToTableToolbar';
...
// ----------------------------------------------------------------------
ToPage.getLayout = (page: React.ReactElement) => <DashboardLayout>{page}</DashboardLayout>;
// ----------------------------------------------------------------------
...
export default function ToPage() {
  ...
  const { data, isLoading, isError, isFetching } = useDmTosQuery({ refetchOnWindowFocus: false });
  useEffect(() => {
  ...
  return (
    <>
      <Head>
        <title>Danh mục | Tổ</title>
      </Head>

      <Container maxWidth={themeStretch ? false : 'xl'}>
        <CustomBreadcrumbs
          heading="Tổ"
          links={[{ name: 'Danh mục', href: PATH_CATEGORY.root }, { name: 'Tổ' }]}
  ...
  if (filterName) {
    ...
    inputData = inputData.filter((user) => {
      ...
      if (!isNil(user?.tenVietTat)) {
        tenVietTatFilter =
          user.tenVietTat.toLowerCase().indexOf(filterNameTrimSpace.toLowerCase()) !== -1;
      }
      let maFilter = false;
      ...
      if (!isNil(user?.phongDai.ten)) {
        tenPhongDaiFilter =
          user.phongDai.ten.toLowerCase().indexOf(filterNameTrimSpace.toLowerCase()) !== -1;
      }
