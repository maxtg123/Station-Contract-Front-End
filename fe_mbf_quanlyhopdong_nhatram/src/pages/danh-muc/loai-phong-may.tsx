...
import {
  LoaiPhongMayCreate,
  LoaiPhongMayTableRow,
  LoaiPhongMayTableToolbar,
} from 'src/sections/category/loai-phong-may';
...
LoaiPhongMayPage.getLayout = (page: React.ReactElement) => (
  <DashboardLayout>{page}</DashboardLayout>
);

// ----------------------------------------------------------------------

export default function LoaiPhongMayPage() {
  const {
    dense,
    page,
    order,
  ...
  const { data, isLoading, isError, isFetching } = useDmLoaiPhongMaysQuery({
    refetchOnWindowFocus: false,
  });

  const { themeStretch } = useSettingsContext();
  const [openPopover, setOpenPopover] = useState<HTMLElement | null>(null);
  const [tableData, setTableData] = useState<IDmLoaiPhongMay[]>([]);
  const [dataRow, setDataRow] = useState<IDmLoaiPhongMay | null>(null);
  ...
  const handleEditRow = (row: IDmLoaiPhongMay) => {
    setDataRow(row);
    setOpenCreate(true);
  };
  return (
    <>
      <Head>
        <title>Danh mục | Loại phòng máy</title>
      </Head>
      <Container maxWidth={themeStretch ? false : 'xl'}>
        <CustomBreadcrumbs
          heading="Loại phòng máy"
          links={[{ name: 'Danh mục', href: PATH_CATEGORY.root }, { name: 'Loại phòng máy' }]}
      ...
      {openCreate && (
        <LoaiPhongMayCreate
          title={dataRow ? 'Cập nhật loại phòng máy' : 'Thêm mới loại phòng máy'}
          open={openCreate}
          // isEdit={isEdit}
          initData={dataRow}
...
function applyFilter({
  ...
  if (filterName) {
    ...
    inputData = inputData.filter((user) => {
      ...
      return maFilter || tenFilter || maDataSiteFilter;
    });
  }
  return inputData;
}
