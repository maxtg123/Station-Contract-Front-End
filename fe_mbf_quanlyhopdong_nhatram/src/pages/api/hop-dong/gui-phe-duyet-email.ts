...
export default async function handler(
  ...
  try {
    ...
    const recipients = _req.body.nguoiPheDuyet;
    const sender = _req.body.nguoiGui;
    ...
    const isPheDuyet = true;
    const urlRedirect = `${process.env.HOSTTING_REDIRECT_ENDPOINT}/${type}/danh-sach/?tab=da_gui_phe_duyet`;
    handlebars.registerHelper('if', function (this: any, conditional, options) {
      if (conditional) {
        return options.fn(this);
      }
    });
    const heading = 'Danh sách hợp đồng cần phê duyệt';
    const title = 'cần phê duyệt từ';
    ...
    const mailPromises = recipients.map(async (recipient) => {
      ...
      const subject = 'Danh sách hợp đồng cần phê duyệt';
      const dataEmail = mailOptions(email, subject, html);
